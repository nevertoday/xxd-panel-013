<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 013 프로젝트 배너" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-수채화-소장-티켓-논리)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 013 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 013 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 013 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 013 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 013의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 013 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 013 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 013 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 013 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 가로 티켓 한 장 · 74/26 분할 · 치유 수채화 · 아이보리 여백 · 현지화 정보 스텁

XXD Panel 013는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 정체성, 실루엣, 자세와 기억을 성립시키는 필수 환경을 보존하고, 그 관계를 고립된 사물 그림이 아니라 중앙에 떠 있는 한 장의 가로 수채화 티켓으로 요약합니다.
...
**Panel 013**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 한 장의 치유 수채화 소장 티켓으로 남깁니다**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 013으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 013이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 한 장의 치유 수채화 소장 티켓으로 남깁니다 |
| **한눈에 알아볼 특징** | 가로 티켓 한 장 · 74/26 분할 · 치유 수채화 · 아이보리 여백 · 현지화 정보 스텁 |
...
```text
주제와 필수 환경 고정 → 원본 고유 단서 최소 세 개 보존 → 중앙의 가로 티켓 한 장으로 요약 → 내부를 약 74/26의 그림과 스텁으로 분할 → 인접 치유색 2–4개로 수채화 번역 → 주제는 조금 선명하게, 환경은 자연스럽게 약화 → 현지화 필드, 원형 도장, 절취선을 하나의 그리드에 정렬
```

무관한 사진으로 바꿔도 수채화 주제, 필수 환경, 팔레트, 필드 의미, 도장 관계와 문구가 거의 그대로 성립한다면 013가 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **주제와 필수 환경:** 최소 세 가지 고유 단서로 정체성, 자세, 크기, 위치, 랜드마크와 공간 관계를 보존합니다.
- **가로 티켓 한 장:** 넓은 아이보리 여백 중앙에 두며 반복, 콜라주, 화면 채우기를 하지 않습니다.
- **약 74/26:** 왼쪽 수채화 면이 확실히 크고, 오른쪽 정보 스텁은 정확한 세로 절취선 하나로 나뉩니다.
- **치유 수채화:** 원본의 고명도·저중채도 인접색 2–4개, 종이 흰색, 번짐, 건습 변화와 소량의 안료 입자를 남깁니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 013의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-013 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-013 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-013 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-013 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-013 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-013 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 013 적용, 공통 설정은 한 번만 확정
/xxd-panel-013 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-013`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-013-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-013-prompt.en.md)
...
**사진 위에 여행 템플릿을 씌우지 말고, 그 관계 자체를 한 장의 티켓으로 남기세요.**
